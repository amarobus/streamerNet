...
def setup(config):
    ...
    lz = config['model']['lz']
    R = config['model']['R']
    n_alpha = config['model']['n_alpha']
    model = CSFNO2d(in_channels, out_channels, modes1, modes2, width, depth, activation=activation, nr=nr, nz=nz, lr=lr, lz=lz, R=R, n_alpha=n_alpha).to('cuda')
...
if __name__ == '__main__':
    ...
    # Start sweep
    sweep_id = wandb.sweep(sweep=config, project="streamernet_test")
